...
_Last reviewed: 2026-06-08 (Agent Skills: enriched SKILL.md discovery — body-fact + bundled-file inventory, plus bundled-script content-scanning for egress/credential reads and hidden-Unicode detection (Tags-block U+E0000–E007F + bidi overrides) — and the `claude_skill` CSKILL-* rule pack (CSKILL-001/002/003/010/011/020/040/050; `schema_version` 10) at the `skill` scope; prior 2026-06-07 review covered JavaScript `.js`/`.jsx`/`.mjs`/`.cjs` via the shared TypeScript-family pipeline and Go/C#/PHP/Rust MCP discovery with field-based rules MCP-015..022)._
