...
_Last reviewed: 2026-06-06 (JavaScript `.js`/`.jsx`/`.mjs`/`.cjs` now parsed via the shared TypeScript-family pipeline and audited by the `language: typescript` rule packs)._
...
> ES-module `import`s are recognized; CommonJS `require()` is a documented gap.
...
now AST-parsed via the shared TypeScript-family pipeline (ES-module imports;
CommonJS `require()` is a documented gap). VAI-011 (HTTP-call-without-timeout)
...
| **Vercel name rules + CommonJS `require()`** | VAI-009 / 010 need a tool `Name` to lint, but Vercel tools take their name from the agent's tools-record key, not the definition — closing this means propagating the record key onto the `ToolDef` during edge resolution. (VAI-011 network-timeout shipped via the structural `has_http_call_without_timeout` predicate; `.js` / `.mjs` / `.cjs` AST parsing shipped via the shared TypeScript-family pipeline.) The remaining JS gap is CommonJS `require()` gating — the import passes recognize only ES `import`, so a `require()`-based `.js`/`.cjs` file parses but triggers no import-gated discovery |
