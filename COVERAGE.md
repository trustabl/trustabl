...
_Last reviewed: 2026-06-11 (Import-alias resolution for the Python body-fact predicates: module and from-import aliases for `subprocess`/`os` (`has_shell_call`), the `eval`/`exec`/`compile` builtins (`has_code_exec_call`), and `requests`/`httpx` HTTP clients (`has_dynamic_url_call` + host/timeout/kwarg machinery) now resolve, closing the common one-line alias evasions found by a detection-quality audit; the deliberate out-of-scope ceiling (local rebinding, dynamic dispatch, aiohttp/urllib aliases) is documented in the Gaps table and pinned by boundary tests. Prior 2026-06-10 Stage 2 typed tool-body captures, cross-cutting Python + TS: `ToolDef.HTTPHosts` — canonical `host:port` of static URL literals on the recognized HTTP-call shapes (scheme default applied, never DNS-resolved) — and `ToolDef.FSWritePaths` — verbatim path literals on the recognized write shapes (`open` with a write mode, `pathlib` `write_text`/`write_bytes`, `shutil` copy/move targets; `fs.writeFile*`/`createWriteStream`); plus a best-effort `retry_present` body fact (tenacity/backoff decorators, client retry kwargs; p-retry/axios-retry/got `retry` option). Static literals only — any interpolation captures nothing and the existing dynamic-URL signals are unchanged. These feed `x-trustabl` surface facts and the OpenShell policy exporter. No rule changes in this pass. Prior 2026-06-08 review below.)_

_Prior review, 2026-06-08 (Agent Skills: enriched SKILL.md discovery — body-fact + bundled-file inventory, plus bundled-file content-scanning for egress/credential reads and committed secret literals, and hidden-Unicode detection (Tags-block U+E0000–E007F + bidi overrides) — and the `claude_skill` CSKILL-* rule pack (CSKILL-001/002/003/010/011/020/030/040/050/060 — 060 = description-vs-tools mismatch; `schema_version` 12; mapped to the OWASP Agentic Skills Top 10 / ASI in the rulebook) at the `skill` scope; plus a deterministic repo-wide **dependency BOM** (`ScanResult.Dependencies` parsed from declared manifests across all six languages — pip/npm/Go/Composer/NuGet/Cargo; optional CycloneDX `--bom-out`; pure inventory by default, TR-278, supersedes the skill-only TR-221) **plus an opt-in `--vuln-scan` layer that matches the repo's concretely-pinned deps against a pinned OSV snapshot and emits CVE/GHSA findings with CVSS-derived severity + fixed version (`internal/vulndb`; `trustabl vulndb pull` pre-warms the cache; snapshot version folded into `ScanID` only when enabled; TR-271)**; prior 2026-06-07 review covered JavaScript `.js`/`.jsx`/`.mjs`/`.cjs` via the shared TypeScript-family pipeline and Go/C#/PHP/Rust MCP discovery with field-based rules MCP-015..022)._
...
| **Static body-fact detection ceiling (import-alias resolution)** | The Python body-fact predicates (`has_shell_call`, `has_code_exec_call`, `has_dynamic_url_call`, and the HTTP host / timeout / call-without-kwarg machinery) match callees structurally and resolve the **common import-alias evasions**: module aliases (`import subprocess as sp` → `sp.run`; `import requests as rq` → `rq.get`; `import os as o`), from-imports (`from subprocess import run as r`; `from builtins import eval as ev`), and module-qualified builtins (`builtins.eval`). **Deliberately out of scope** — the honest static-detection ceiling — are: local rebinding (`_e = eval; _e(x)`), fully dynamic dispatch (`getattr(builtins, "ev"+"al")`, `__import__("os").system`), aliased `aiohttp`/`urllib` modules and HTTP `from`-imports (`from requests import get`), and any string-constructed call. These are pinned by boundary tests (e.g. `TestPred_HasCodeExecCall_LocalRebindOutOfScope`) so closing one is a deliberate decision, not drift. An AST predicate cannot catch a determined obfuscator; chasing every shape is whack-a-mole. The per-rule "what this does not cover" sections in the **rulebook** are the deeper home for this caveat |
