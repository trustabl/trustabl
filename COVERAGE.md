...
_Last reviewed: 2026-06-05 (forward-compatible rule loading; TS network-timeout predicate — OAI-016 retrofit + VAI-011 shipped)._
...
so there is nothing to lint) are not shipped; `.js` / `.mjs` apps are recognized
in the inventory but not AST-parsed (the TS parser only handles `.ts` / `.tsx`).
VAI-011 (HTTP-call-without-timeout) now ships via the structural
