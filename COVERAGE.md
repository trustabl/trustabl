...
kwarg or the captured positional.
...
| **LangChain class tools + raw `StateGraph` + TS provider hosted tools** | Three additive discovery gaps in the LangChain pack: `class X(BaseTool)` / `extends StructuredTool` (a class shape, not a call); the raw `StateGraph` → `.compile()` graph agent — **Python discovery has landed** (`langgraph_graph.go`: the agent is discovered and the SDK marked; remaining is resolving its `ToolNode` / `bind_tools` tools, shipping `langchain_state_graph` rules, and the TS equivalent); and the date-stamped TS provider hosted tools (`shell()` / `bash_*` / `applyPatch`). Each is discovery-only, no schema change |
...
5. **LangChain / LangGraph pack** has **landed** (newest SDK row): 15 rules
   ...
   file. The raw `StateGraph` → `.compile()` graph agent is now **discovered** in
   Python (`langgraph_graph.go`; its `ToolNode` / `bind_tools` tool-edges and the
   `langchain_state_graph` rules are pending, as is the TS equivalent). Remaining
