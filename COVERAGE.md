...
_Last reviewed: 2026-06-10 (Stage 2 typed tool-body captures, cross-cutting Python + TS: `ToolDef.HTTPHosts` — canonical `host:port` of static URL literals on the recognized HTTP-call shapes (scheme default applied, never DNS-resolved) — and `ToolDef.FSWritePaths` — verbatim path literals on the recognized write shapes (`open` with a write mode, `pathlib` `write_text`/`write_bytes`, `shutil` copy/move targets; `fs.writeFile*`/`createWriteStream`); plus a best-effort `retry_present` body fact (tenacity/backoff decorators, client retry kwargs; p-retry/axios-retry/got `retry` option). Static literals only — any interpolation captures nothing and the existing dynamic-URL signals are unchanged. These feed `x-trustabl` surface facts and the OpenShell policy exporter. No rule changes in this pass. Prior 2026-06-08 review below.)_
